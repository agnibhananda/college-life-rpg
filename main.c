...
int crescendo() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        snprintf(command, sizeof(command), "mpg123 %s", files[i]);
        printf("Playing: %s\n", songs[i]);
        system(command);  
...
//code for debsoc club
void displayQuestion(int questionNumber, char question[], char options[][100], char correctAnswer) {
...
int debsoc() {
    ...
    char options[10][4][100] = {
        {"Lord Mountbatten", "C. Rajagopalachari", "Jawaharlal Nehru", "Rajendra Prasad"},
    ...
    char answers[] = {'B', 'B', 'A', 'B', 'B'}; // Correct answers
    ...
    for (int i = 0; i < totalQuestions; i++) {
        printf("\n");
        displayQuestion(i + 1, questions[i], options[i], answers[i]);
    }
...
void askPuzzle(int puzzleNumber, const char *puzzle, const char *correctAnswer) {
    char answer[100];
    printf("\nPuzzle %d: %s\n", puzzleNumber, puzzle);
    printf("Your answer: ");
    fgets(answer, sizeof(answer), stdin);
...
int KPH() {
    ...
    for (int i = 0; i < totalPuzzles; i++) {
        askPuzzle(i + 1, puzzles[i], answers[i]);
        printf("\n");
    }
...
// Added Save/Load and Hints Functionality
...
bool loadGame(const char *filename) {
    ...
    while (fgets(line, sizeof(line), file)) {
        // Parse the file and update the game state
        ...
        // Add similar parsing for inventory and quests
    }
...
// Function to provide hints based on game state
void provideHints() {
    printf("\nHints:\n");
    
    // Suggest completing an incomplete quest
    for (int i = 0; i < questLog.count; i++) {
        if (!questLog.quests[i].completed) {
            printf("- Complete the quest: %s\n", questLog.quests[i].name);
            break;
        }
    }
    
    // Suggest resting if energy is low
    if (player.energy < 10) {
        printf("- Your energy is low. Consider resting.\n");
    }
    
    // Suggest studying if intelligence is low
    if (player.intelligence < 5) {
        printf("- Your intelligence is low. Consider studying.\n");
    }
}
